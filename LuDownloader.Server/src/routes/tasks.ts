...
export function registerTasksRoutes(app: Hono): void {
  app.get('/api/tasks', apiKeyAuth, async (c) => {
  ...
  app.post('/api/tasks', apiKeyAuth, async (c) => {
  ...
  app.patch('/api/tasks/:id', apiKeyAuth, async (c) => {
  ...
  app.delete('/api/tasks/:id', apiKeyAuth, async (c) => {
    const id = c.req.param('id')
    const [task] = await db
      .delete(tasks)
